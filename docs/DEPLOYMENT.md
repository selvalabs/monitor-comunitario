...
When the resident replies `OK`, `CANCELAR`, or `PARAR`, Hermes calls:
...
Only exact `OK` activates the phone, enables notifications, and creates the member record. `CANCELAR` removes the pending registration; no response lets the Redis request expire after 48 hours. `PARAR` keeps an active member account but disables future outage notifications. After confirmation, Monitor creates a `member_phone_confirmation_completed` event for Hermes to send the approved access-code message.
